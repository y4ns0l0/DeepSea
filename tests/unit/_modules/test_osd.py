...
class TestOSDPartitions():
    ...
    @patch('srv.salt._modules.osd._run')
    @mock.patch('srv.salt._modules.osd._find_paths')
    def test_clean(self, mock_fp, mock_r):
        osd_config = OSDConfig()

        osdp = osd.OSDPartitions(osd_config)
        mock_fp.return_value = ['/dev/sda1']
        mock_r.return_value = (0, "out", "err")
        osdp.clean()
        assert mock_fp.call_count == 1
...
class TestOSDRemove():
    ...
    @patch('srv.salt._modules.osd.OSDRemove.set_partitions')
    ...
    @patch('srv.salt._modules.osd._run')
    @patch('time.sleep')
    def test_terminate(self, mock_sleep, mock_r):
        partitions = {'osd': '/dev/sda1'}
        mock_device = mock.Mock()
        mock_device.partitions.return_value = partitions

        osdr = osd.OSDRemove(1, mock_device, None, None)
        mock_r.return_value = (1, "out", "err")
        result = osdr.terminate()
        assert result == ""

    @patch('srv.salt._modules.osd._run')
    @patch('time.sleep')
    def test_terminate_fails(self, mock_sleep, mock_r):
        partitions = {'osd': '/dev/sda1'}
        mock_device = mock.Mock()
        mock_device.partitions.return_value = partitions
    ...
    @patch('os.rmdir')
    @patch('__builtin__.open', new=f_open)
    def test_unmount(self, mock_rmdir, mock_r):
    ...
    @patch('srv.salt._modules.osd._run')
    @patch('os.path.exists', new=f_os.path.exists)
    def test_wipe_fails(self, mock_r):
        TestOSDRemove.fs.CreateFile('/dev/sda1')
        partitions = {'osd': '/dev/sda1'}
        mock_device = mock.Mock()
        mock_device.partitions.return_value = partitions
    ...
    @patch('srv.salt._modules.osd.readlink')
    @patch('os.path.exists', new=f_os.path.exists)
    def test_delete_partitions_with_standalone_osd(self, mock_rl, mock_r):
        TestOSDRemove.fs.CreateFile('/dev/sda1')
        partitions = {'osd': '/dev/sda1'}
        mock_device = mock.Mock()
        mock_device.partitions.return_value = partitions
    ...
    @patch('srv.salt._modules.osd.readlink')
    @patch('os.path.exists', new=f_os.path.exists)
    def test_delete_partitions_working(self, mock_rl, mock_sp, mock_r):
    ...
    @patch('srv.salt._modules.osd._run')
    def test_mark_destroyed(self, mr):
        partitions = {'osd': '/dev/sda1'}
        mock_device = mock.Mock()
        mock_device.partitions.return_value = partitions
        ...
        mr.return_value = (0, "out", "err")

        result = osdr.mark_destroyed()
        assert result == True

    @patch('srv.salt._modules.osd._run')
    def test_mark_destroyed_with_keyring_and_client(self, mr):
        partitions = {'osd': '/dev/sda1'}
        mock_device = mock.Mock()
        mock_device.partitions.return_value = partitions

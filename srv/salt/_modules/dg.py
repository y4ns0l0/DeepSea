...
class Inventory():
    ...
    def raw(self) -> str:
        ...
        if return_code == 0:
            return stdout
        log.error('ceph-volume inventory --format json returned with {}'.format(return_code))
        log.error(stderr)
        log.error(stdout)
        return '{}'
...
class Matcher(object):
    ...
    def _virtual(self):
        ...
        """
        virtual: dict = __grains__['virtual']
        if virtual != "physical":
...
def test(**kwargs):
    ...
    print(dgo.data_devices)
    print(dgo.wal_devices)
    print(dgo.db_devices)
    return dict(
